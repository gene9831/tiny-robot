 ...
 */
export const ensureZeroWidthAroundField = (fieldElement: HTMLElement): void => {
  // 检查字段内容是否为空
  if (isFieldEmpty(fieldElement)) {
    // 如果是空的，确保有零宽字符
    if (!fieldElement.textContent || fieldElement.textContent === '') {
      fieldElement.textContent = ZERO_WIDTH_SPACE
    }
  }
