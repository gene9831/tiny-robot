...
test.describe('ExtensionManager namespace', () => {
  test('exposes only the current Manager, Card, and CardGrid facade', async ({ mount }) => {
    ...
    await expect(component.getByTestId('detail-exported')).toHaveText('false')
    await expect(component.getByTestId('form-exported')).toHaveText('false')
    ...
    await expect(component.getByTestId('detail-registration')).toHaveText('false')
    await expect(component.getByTestId('form-registration')).toHaveText('false')
