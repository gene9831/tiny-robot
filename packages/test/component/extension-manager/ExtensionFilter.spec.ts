...
test.describe('ExtensionManager.Filter', () => {
  test('exposes the complete canonical projection while no Filter is mounted', async ({ mount }) => {
    ...
    await expect(component.getByTestId('display-available')).toHaveText('Train service,Translate skill,Research skill')
  })
  ...
  test('filters installed and available items by the selected tag', async ({ mount }) => {
    const component = await mount(ExtensionFilterFixture)
    await component.getByTestId('toggle-filter').click()
  ...
  test('filters installed and available items by the query', async ({ mount }) => {
    const component = await mount(ExtensionFilterFixture)
    await component.getByTestId('toggle-filter').click()
  ...
  test('clears controlled query and tag models when the active type changes', async ({ mount }) => {
    ...
    await expect(component.getByTestId('display-available')).toHaveText('Translate skill,Research skill')
  })
